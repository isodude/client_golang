...
import (
	...
	"github.com/isodude/client_golang/api"
)
...
type API interface {
	...
	Federate(ctx context.Context, matches []string, startTime, endTime time.Time, opts ...Option) (model.Value, Warnings, error)
...
func (h *httpAPI) Federate(ctx context.Context, matches []string, startTime, endTime time.Time, opts ...Option) (model.Value, Warnings, error) {
	u := h.client.URL(apiFederate, nil)
	q := u.Query()
